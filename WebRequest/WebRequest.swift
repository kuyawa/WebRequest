...
class WebRequest {
    ...
    // Post text as body
    // web.post("http://example.com", body: "Lorem ipsum dolor sit amet") { response in }
    func post(_ url: String, body: String, callback: @escaping Callback) throws {
        try? self.post(URL(string: url)!, body: body, callback: callback)
    }

    func post(_ url: URL, body: String, callback: @escaping Callback) throws {
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.httpBody = body.data(using: .utf8)
        parseHeaders(&request)
    ...
    // web.put("http://example.com", body: "Lorem ipsum dolor sit amet") { response in }
    func put(_ url: String, body: String, callback: @escaping Callback) throws {
        try? self.put(URL(string: url)!, body: body, callback: callback)
    }

    func put(_ url: URL, body: String, callback: @escaping Callback) throws {
        var request = URLRequest(url: url)
...
// Web.get("http://example.com") { response in ... }
class Web {
    static var session = WebRequest()
    
    static func get(_ url: String, callback: @escaping Callback) throws {
        try? session.get(URL(string: url)!, params: [:], callback: callback)
    }
